...
/* modified by jovi: multipurpose support */
void build_orig_constant_list( void ) {
    ...
    /* jovi: support multiple purpose planning*/
    /* collect the addtional ops */
    for ( pao = gadd_loaded_ops; pao; pao = pao->next ) {
        collect_type_names_in_pl( pao->preconds );
        collect_type_names_in_pl( pao->effects );
    ...
    prinf("DebuInfo: finish gadd_load_ops \n");
