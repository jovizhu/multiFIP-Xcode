...
/* add the fact into list lF and set gft_conn to be true*/
...
/* add the effect into list LE and set gef_conn to be true*/
...
void reset_relax( void ){
    ...
    for ( i = 0; i < gnum_ef_conn; i++ ) {
        ...
        if(gef_conn[i].num_PC == 0){/*// op without preconditions*/
            l0P_E[lnum_0P_E++] = i;
        }		
    }
