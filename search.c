...
********************************/
Bool do_weak_planning( State *start, State *end ) {
	/*
	printf("debug: subplanning---initial state\n");
	debugit(start);
	printf("\ndebug: goal state\n");
	debugit(end);
	printf("\n*************************\n");
	...
	*/
	Bool found_plan = do_enforced_hill_climbing(start, end);
	if(!found_plan){
...
/*copy funciton: source to destination*/
...
void reset_search( void ){
	...
	for ( i = 0; i < EHC_HASH_SIZE; i++ ) {
		...
		free_StateHashEntryList( lehc_hash_entry[i] );		
		lehc_hash_entry[i] = NULL;
		lnum_ehc_hash_entry[i] = 0;
		lchanged_ehc_entrys[i] = FALSE;
