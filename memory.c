...
PlanNode* new_PlanNode( void ){
	int i;
	
	PlanNode *p = ( PlanNode * ) calloc( 1, sizeof( PlanNode ) );
	CHECK_PTR(p);
	p->num_sons	=	0;
	p->action	=	-1;
	p->id			= 	-1;
...
StateActionPair* new_StateActionPair( void ){
...
void free_StateActionPair( StateActionPair* p ){
	if( p ){
		free(p);
	}
}
...
void free_StateHashEntryList( EhcHashEntry * p ){
	EhcHashEntry *pp;
	while( p ){		
		pp = p->next;
		free(p);
